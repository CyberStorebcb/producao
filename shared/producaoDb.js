...
async function loadRowsFromDb(client, sheetName) {
...
async function loadNormalizedSheetFromDb(client, sheetName) {
  const rows = await loadRowsFromDb(client, sheetName);
  return {
    rows,
    normalized: buildNormalizedFromRows(rows, sheetName),
  };
}
